...
export async function PATCH(
  ...
  if (joinRequest.trip.organizerId !== session.user.id) {
    return NextResponse.json(
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (freshTrip.currentMembers >= freshTrip.maxCapacity) {
      ...
      const updatedRequest = await tx.joinRequest.update({
        ...
        include: {
          user: { select: { id: true, name: true, email: true } },
          trip: { select: { id: true, title: true, currentMembers: true, maxCapacity: true } },
        },
      });

      // Step 4: Increment the trip's member count atomically.
      //   { increment: 1 } compiles to: SET currentMembers = currentMembers + 1
